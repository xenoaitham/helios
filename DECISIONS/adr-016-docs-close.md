...
| A1: re-execute `make clean && make up` on the living stack today | Destroys the measured build history (corpora, snapshot invariant, 8 RESOLVED incidents, CDC slot) to re-prove a Phase-0 result; every "bit-identical since 2026-09-11" claim in EVIDENCE dies with it | Rejected - the cost is permanent and the information gain is zero |
